...
* **引用类型**
  ...
  * **强引用（Strong Reference）** 
    * 最普遍或者“传统”的定义，指在程序代码中的普通存在的赋值引用，如：`Object obj = new Object()`中的`obj`引用。
    * **垃圾回收宁愿OOM也不回收强引用所指向的对象**。

  * **软引用（Soft Reference）**
    * **Reference**的子类， 在系统即将要内存溢出前，会将这些对象列入回收范围中进行二次回收。二次回收后内存还够，就OOM了。
    * 软引用一般用来实现内存敏感的缓存，比如很多高速Cache，内存足保留缓存，内存不足清理掉。
    * 软引用可以配合**引用队列**（Reference Queue）来使用，垃圾回收软引用时，会清理软引用，并可选的把地址放到**引用队列**中。

  * **弱引用（Weak Reference）**
    * **Reference**的子类，被弱引用关联的对象只能存活到下一次GC之前，不管内存空间够不够，只要进行GC这些对象就会被回收。
    * 弱引用跟软引用一样也能指定**引用队列**（Reference Queue），这样当弱引用被回收时可以加入到**引用队列**中，通过队列来跟踪垃圾回收情况。
    * 弱引用也非常适合来缓存那些可有可无的数据的场景，如： ThreadLocalMap中的Entry<?>对象。
    * **弱引用与软引用的区别是：GC碰到弱引用就回收；碰到软引用还犹豫，二次再回收。**

  * **虚引用（Phantom Reference）**
    * **Reference**的子类，一个对象是否有虚引用存在，完全不会对其生存产生影响，也无法通过虚引用来获得一对象的引用。
  为一个对象设置虚引用的唯一目的就是能在这个对象被GC回收时给得到一个系统通知。
    * **虚引用必须和引用队列一起使用。**

* **引用终结器：FinalReference**
  * `FinalReference`内部有引用队列，用来执行`finalize()`方法，GCl进行回收时，进入引用队列， 通过`FinalReference`找到被引用对象并调用执行`finalize()`方法 。由Finalizer线程
...
* 适用于**垃圾清除阶段**的算法
  ...
  **复制**、**标记-清除**、**标记-压缩**三种是JVM目前比较常用的垃圾收集算法。
  ...
  * **复制（Copying）算法**
    * **核心**：把活着的内存分为两，每次只使用一块，增加收回时将存活对象全都复制到一块内存中，另一块
内存的存活对象全清空，如此反复交替，完成垃圾回收。
    * **优点**：没有标记和清除过程，简单高效；不会出现连续内存碎片
    * **缺点**：浪费一半空间，对于G1这种针对region的gc，是复制而不是移动，需要维护不同region之间的引用，开销也很大。
...
* **基于**对象生命周期**的垃圾回收算法**：
  ...
  * **分代收集算法**： 目前几乎所有的GC都是基于分代收集算法，再根据不同代中特性使用其他垃圾回收算法。
    * 年轻代（Young Gen）: 对象生命周期短，存活率小，回收频繁
    * 老年代（Tenured Gen）：内存区域大，生命周期长，存活率高，回收不及年轻代频繁
    
* **分区算法**：典型G1回收使用的垃圾回收算法，关注的是**目标的GC停顿时长**。
  * 将一块大的内存区域分成许多小的内存块，根据目标的STW停顿时间，每次合理的收集若干区域，而不是每回收整个内存区域。

* **增量收集算法**：
  * 该算法基础是 标记-清除和复制算法。 
  * 如果一次性将所有的垃圾进行处理，需要造成系统长时间停顿，**增量收集算法**让GC线程和应用线程交替执行，
GC线程每次只回收一小部分内存，然后切换到应用线程。依此反复，直到垃圾收集完成。
  * 缺点是会影响整个系统的吞吐。
